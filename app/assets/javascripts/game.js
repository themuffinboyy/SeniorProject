...
var difficulty;
var numOfClicks = 0;
var numOfCleanCells; //Spaces that aren't uncovered yet and don't contain a mine
var gameBoard = [];
var score = 0;
var winner = 0;
var hintsUsed = 0;
...
var time;
var skin;
var is_flag = false; // Used to tell if player is clicking or changing space to flag
...
function create(rows, columns, bombs, difficulty) {
	...
	numOfcleanCells = (rows*columns)-(bombs);
...
// Sets all the bombs across the board
function populate(bombs) {
	left = bombs;
	...
	$(".closed").each(function() {
		...
		if(Math.random() < 0.01 && left > 0 && gameBoard[pos[0]][pos[1]] == false) {
			left--;
			gameBoard[pos[0]][pos[1]] = true;
...
function clickedOn(position) {
	...
	if (is_flag == false) {
		...
		else if (gameBoard[pos[0]][pos[1]] && finder(pos).className != "closed " + skin + " flag" && finder(pos).innerHTML != "F" ) {
	    lose();
		}
		...
		//if it's not a mine or the first click, open the square
		  ...
		  if (numOfcleanCells == 0) {
		    win();
		  }

		  if (checkMines(pos[0],pos[1]) == "") {
		  	blankSpaces.push([pos[0],pos[1]]);
		  	blankOpen();
		  }
		}
	}
		...
		for (var i = -1; i < 2; i++) {
			...
			for (var j = -1; j < 2; j++) {
				...
				if (document.getElementById(position)) {
					...
					if (square.classList.contains("closed") && gameBoard[position[0]][position[1]] == false) {
						...
						if (checkMines(position[0],position[1]) == "") {
							blankSpaces.push([position[0],position[1]]);
						}
					}
				}
...
function checkMines(x,y) {
	...
	for (var iCM = -1; iCM < 2; iCM++) {
		...
		for (var jCM = -1; jCM < 2; jCM++) {
			...
			if(document.getElementById(posit2) && gameBoard[pos2[0]][pos2[1]])  {
				numMines++;
			}
...
// Shows a square that is safe to click on
function hint() {
	if (numOfClicks > 0) {
	  hintsUsed++;
	  allPossible = [];
	  ...
	  $(".closed").each(function() {
		...
		if (gameBoard[pos[0]][pos[1]] == false) {
